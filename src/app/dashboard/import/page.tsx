...
export default function ImportPage() {
  ...
  async function syncMaintenant() {
    ...
    try {
      ...
      alert(data.totalImported > 0
        ? `✅ ${data.totalImported} nouvelle(s) commande(s) importée(s) !`
  ...
  function matchProduit(productName: string): any | null {
    ...
    return produits.find((p: any) => {
      const pn = p.nom.toLowerCase()
      return pn === name || pn.includes(name.slice(0, 8)) || name.includes(pn.slice(0, 8))
    }) || null
  }
  ...
  async function fetchSheet() {
    ...
    if (csvUrl.includes('spreadsheets/d/')) {
      ...
      csvUrl = `/api/sheets?url=${encodeURIComponent(csvUrl)}`
    }
    ...
    try {
      ...
      const text = json.csv || ''
      if (!text) throw new Error('Fichier vide ou format invalide.')
      ...
      const parsed: SheetRow[] = lines.slice(1).map((line: string) => {
        ...
        if (derniereSync && rowDate && !isNaN(rowDate.getTime()) && rowDate <= derniereSync) status = 'duplicate'
  ...
  function toggleRow(i: number) {
    setRows(prev => {
      const u = [...prev]
      u[i] = { ...u[i], status: u[i].status === 'selected' ? 'new' : 'selected' }
      return u
    })
  }
  ...
  async function importerSelection() {
    if (!userId) return
    const toImport = rows.filter(r => r.status === 'new' || r.status === 'selected')
    if (toImport.length === 0) { alert('Aucune commande à importer'); return }
    ...
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i]
      if (row.status !== 'new' && row.status !== 'selected') continue
      try {
        ...
        const { data: cmd } = await supabase.from('commandes').insert({
          user_id: userId, client_id: clientId, zone_id: zoneId, statut: 'en_attente',
          cout_livraison: coutLiv, notes: `Import manuel Shopify — ${row.date_time}`,
        }).select('id').single()

        if (cmd) {
          await supabase.from('commande_items').insert({
            commande_id: cmd.id, produit_id: produit?.id || null,
      ...
      } catch { updated[i] = { ...row, status: 'error' } }
      setRows([...updated]); setDone(count)
      await new Promise(r => setTimeout(r, 60))
    }

    await supabase.from('sync_config').update({
      derniere_sync: new Date().toISOString(),
      nb_importees: (syncConfig?.nb_importees || 0) + count,
      updated_at: new Date().toISOString()
    }).eq('user_id', userId)

    setImporting(false); setImportDone(count); setStep('done')
  }
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', paddingBottom: 48 }}>
      <style>{`
        ...
        .sbt{transition:all .15s;cursor:pointer;font-family:inherit;}
        .row-card{transition:border-color .15s;}
        .row-card:hover{border-color:#CECBF6!important;}
      `}</style>

      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#0C0C1E', letterSpacing: -.5, margin: 0 }}>Sync Shopify</h1>
        <p style={{ fontSize: 13, color: '#ABABAB', marginTop: 4 }}>Importe tes commandes automatiquement</p>
      </div>

      {/* TABS */}
      <div style={{ display: 'flex', gap: 4, background: '#F0F0F8', borderRadius: 14, padding: 4, marginBottom: 20 }}>
        {[['auto','⚡ Automatique'],['manuel','📥 Manuel']].map(([id, label]) => (
      ...
      {tab === 'auto' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Statut */}
          <div style={{ background: syncConfig?.actif ? '#0C0C1E' : '#F8F8FC', borderRadius: 18, padding: '20px', border: syncConfig?.actif ? 'none' : '1px solid #EBEBEB' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: syncConfig?.actif ? 12 : 0, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: syncConfig?.actif ? '#fff' : '#0C0C1E' }}>
                  {syncConfig?.actif ? 'Synchronisation active' : 'Synchronisation inactive'}
                </span>
              </div>
              {syncConfig?.actif && (
                <button className="sbt" onClick={desactiverSync}
                  style={{ background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.15)', color: 'rgba(255,255,255,.5)', borderRadius: 10, padding: '6px 12px', fontSize: 12, fontWeight: 600 }}>
                  Désactiver
                </button>
              )}
            </div>
            {syncConfig?.actif && (
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                <div>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,.35)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 3 }}>Dernière sync</p>
                ...
                <div>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,.35)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 3 }}>Total importé</p>
          ...
          <div style={{ background: '#fff', borderRadius: 18, padding: '20px', border: '1px solid #F0F0F0', boxShadow: '0 1px 8px rgba(0,0,0,.04)' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#0C0C1E', marginBottom: 16 }}>Configuration</p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button className="sbt" onClick={sauvegarderSync} disabled={savingSync || !sheetUrl}
                ...
                {savingSync ? '⏳...' : syncSaved ? '✓ Activée !' : '💾 Activer la sync'}
              </button>
              {syncConfig && (
                <button className="sbt" onClick={syncMaintenant} disabled={syncing}
                  ...
                  {syncing ? <span style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid #16A34A', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .6s linear infinite' }} /> : '▶ Sync maintenant'}
                </button>
              )}
            </div>
          </div>
          ...
          {!produits.length && (
            <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 14, padding: '12px 16px' }}>
              ...
              <p style={{ fontSize: 12, color: '#DC2626', marginTop: 4 }}>
                Importe tes produits depuis <a href="/dashboard/import-produits" style={{ color: '#DC2626', fontWeight: 700 }}>Import Produits</a> pour lier les commandes à tes produits.
              </p>
            </div>
          )}
        </div>
      ...
      {tab === 'manuel' && step === 'config' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: '20px', border: '1px solid #F0F0F0', boxShadow: '0 1px 8px rgba(0,0,0,.04)' }}>
            <p style={{ fontSize: 15, fontWeight: 700, color: '#0C0C1E', marginBottom: 16 }}>Importer des commandes</p>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#A0A0A0', textTransform: 'uppercase', letterSpacing: '.07em', display: 'block', marginBottom: 6 }}>URL Google Sheet ou CSV</label>
              <input style={F} value={sheetUrl} onChange={e => setSheetUrl(e.target.value)} placeholder="https://docs.google.com/spreadsheets/d/..." />
            </div>
            <div style={{ marginBottom: 16 }}>
            ...
            <button className="sbt" onClick={fetchSheet} disabled={loading || !sheetUrl}
              style={{ width: '100%', background: 'linear-gradient(135deg,#7F77DD,#534AB7)', color: '#fff', border: 'none', borderRadius: 12, padding: '13px', fontSize: 15, fontWeight: 700, opacity: !sheetUrl ? .5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              {loading
                ? <><span style={{ width: 18, height: 18, border: '2px solid #fff', borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'spin .6s linear infinite' }} />Lecture...</>
      ...
      {tab === 'manuel' && step === 'preview' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Stats */}
          <div style={{ background: '#0C0C1E', borderRadius: 18, padding: '18px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
              <div>
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 4 }}>Commandes trouvées</p>
                <p style={{ fontSize: 42, fontWeight: 800, color: '#fff', letterSpacing: -2, lineHeight: 1 }}>{rows.length}</p>
              </div>
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: 24, fontWeight: 800, color: '#9FE1CB' }}>{newRows.length}</p>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,.4)' }}>Nouvelles</p>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: 24, fontWeight: 800, color: '#FAC775' }}>{dupRows.length}</p>
          ...
          {importing && (
            <div style={{ background: '#fff', borderRadius: 14, padding: '14px 16px', border: '1px solid #EBEBEB' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 13, color: '#7F77DD', fontWeight: 700 }}>{done}/{newRows.length}</span>
              </div>
              <div style={{ background: '#F0F0F8', borderRadius: 8, height: 8, overflow: 'hidden' }}>
                <div style={{ width: `${newRows.length > 0 ? (done / newRows.length) * 100 : 0}%`, height: '100%', background: 'linear-gradient(90deg,#7F77DD,#1D9E75)', borderRadius: 8, transition: 'width .3s' }} />
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button className="sbt" onClick={() => setRows(prev => prev.map(r => r.status === 'duplicate' || r.status === 'imported' ? r : { ...r, status: 'selected' }))}
            ...
            <button className="sbt" onClick={() => setRows(prev => prev.map(r => r.status === 'duplicate' || r.status === 'imported' ? r : { ...r, status: 'new' }))}
            ...
            {rows.map((r, i) => {
              ...
              return (
                <div key={i} className="row-card"
                  ...
                  style={{ background: isImported ? '#F0FFF4' : isDup ? '#FFFBEB' : isSelected ? '#EEEDFE' : '#fff', border: `1.5px solid ${isImported ? '#BBF7D0' : isDup ? '#FDE68A' : isSelected ? '#7F77DD' : '#EBEBEB'}`, borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, cursor: isDup || isImported ? 'default' : 'pointer', opacity: isDup ? .6 : 1 }}>
                  {!isDup && !isImported && (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {r.product_name && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                        <span style={{ fontSize: 11, color: r.matched_produit ? '#16A34A' : '#D97706', fontWeight: 600 }}>🛍️ {r.product_name}</span>
                        <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 20, background: r.matched_produit ? '#F0FFF4' : '#FFFBEB', color: r.matched_produit ? '#16A34A' : '#D97706', fontWeight: 700 }}>
                          {r.matched_produit ? '✓ Lié' : 'Sans produit'}
                        </span>
                      </div>
                    )}
          ...
          <button className="sbt" onClick={importerSelection} disabled={importing || newRows.length === 0}
            style={{ width: '100%', background: 'linear-gradient(135deg,#7F77DD,#534AB7)', color: '#fff', border: 'none', borderRadius: 16, padding: '15px', fontSize: 15, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 4px 20px rgba(127,119,221,.4)', opacity: newRows.length === 0 ? .5 : 1 }}>
            {importing
              ...
              : `⚡ Importer ${rows.filter(r => r.status === 'selected').length || newRows.length} commande${newRows.length > 1 ? 's' : ''}`}
          </button>
        </div>
      )}
      ...
      {tab === 'manuel' && step === 'done' && (
        <div style={{ background: '#fff', borderRadius: 20, padding: '48px 24px', textAlign: 'center', border: '1px solid #EBEBEB' }}>
          ...
          <p style={{ fontSize: 22, fontWeight: 800, color: '#0C0C1E', letterSpacing: -.5, marginBottom: 8 }}>{importDone} commande{importDone > 1 ? 's' : ''} importée{importDone > 1 ? 's' : ''} !</p>
          <p style={{ fontSize: 14, color: '#ABABAB', marginBottom: 32 }}>Retrouve-les dans ta page Commandes.</p>
